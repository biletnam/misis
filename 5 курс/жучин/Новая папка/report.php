...
<form method="post">
	<table>
		...
		<tr>
			...
			<td><input value="<?=$_POST['date_start']?>" class="datepicker_edit" type="text" size="25" name="date_start" id="date_start"></td>
		</tr>
		
		<tr>
			<td>Конец периода</td>
			<td><input value="<?=$_POST['date_end']?>" class="datepicker_edit" type="text" size="25" name="date_end" id="date_end"></td>
		</tr>
		
		<tr>
			<td>&nbsp;</td>
			<td><input type="submit" style="width: 100%;" name="report" value="Отчет"></td>
		</tr>
	</table>
</form>
<br>
	...
	if (isset($_POST['report']))
	{
		...
		if (mysql_num_rows($sql))
		{
			...
			<tr>
				...
				<td><?=conv_date($res['protocol_date'])?></td>
				<td><?=conv_date($res['select_date'])?></td>
				<td><?=conv_date($res['test_date'])?></td>
				<td><?=$client?></td>
...
<?
	function conv_date($str)
	{
		...
		if ($date == '01.01.70') $date = 'данные отсутствуют';
		return $date;
	}
?>