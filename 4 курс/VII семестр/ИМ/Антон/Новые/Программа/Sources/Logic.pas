...
begin
  ...
  begin
    AddRequest(RandomRange(Round(Settings.MinRequest.Value/100),Round(Settings.MaxRequest.Value/100))*100);
  end;
end;
...
begin
   ...
   MinRequest := TIntegerInput.Create(300,Panel);
   ...
   MaxRequest := TIntegerInput.Create(500,Panel);
   ...
   TimeRange := TIntegerInput.Create(30,Panel);
...
begin
  ...
  lbl1.Parent := Panel;
  lbl1.AutoSize := true;
  lbl1.Caption := cName+':    Грузоподъемность';
  lbl1.Top := 5;
  lbl1.Left := 5;

  MassSelect := TIntegerInput.Create(1500,Panel);
