...
Type TCar=Class(TObject)
  ...
  procedure CreateSettings(cParent: TWinControl);
  procedure NewMaxMass(Sender: TObject);
  procedure Reset;
end;
...
begin
  ...
  begin
    ...
    if (WaitTime+WorkTime)<>0 then kpd:=100-Round(100*WorkTime/(WaitTime+WorkTime))
    else kpd:=0;
...
begin
  ...
  InWay := true;
  СurrentMass := cMass;
  Log.Add(Name + ' отправляется в ' + cClient.Name +'. Заправлено: '+IntToStr(СurrentMass)+' л.');
end;
...
begin
  ...
  begin
    Log.Add(Name + ' прибыла в ' + Client.Name +'. Доставлено: '+IntToStr(СurrentMass)+' л.');
    TimeRemain := -1;
    InWay := false;
...
begin
  with Controls do
  begin
    NeedMassInfo.Edit.Text := IntToStr(NeedMass) + ' л.';
...
begin
  ...
  Log.Add('Поступил заказ от ' + Name + ' на ' + IntToStr(Mass)+' л. бензина.');
end;
...
begin
  ...
  begin
    AddRequest(RandomRange(Round(Settings.MinRequest.Value/1000),Round(Settings.MaxRequest.Value/1000))*1000);
  end;
end;
...
begin
  ...
  Settings.MassSelect.Edit.OnChange := NewMaxMass;
end;
...
begin
  ...
  lbl1.Parent := Panel;
  lbl1.AutoSize := true;
  lbl1.Caption := cName+':    Вместимость';
  lbl1.Top := 5;
  lbl1.Left := 5;

  MassSelect := TIntegerInput.Create(10000,Panel);
  MassSelect.Edit.Top := 0;
  MassSelect.Edit.Left := lbl1.Left + lbl1.Width + 5;
  ...
  lbl2.Caption := 'л.';
  lbl2.Top := lbl1.Top;
  lbl2.Left := MassSelect.Edit.Left+MassSelect.Edit.Width+5;
...
procedure TCar.NewMaxMass(Sender: TObject);
begin
  MaxMass := Settings.MassSelect.Value;
end;
...
begin
   for i := 0 to ClientMan.Clients.Count - 1 do
   begin
      buf := ClientMan.Clients[i] as TClient;
      buf.Reset;
   end;
   for i := 0 to CarMan.Cars.Count - 1 do
   begin
