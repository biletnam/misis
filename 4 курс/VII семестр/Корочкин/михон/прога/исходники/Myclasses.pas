...
public
  ...
  Property MoneyInfo: TInfoRecord read mMoneyInfo write mMoneyInfo;
  ...
  procedure DowntTo(IC: TInfoControls; delta: integer);
end;
...
begin
  ...
  mStateInfo := TInfoRecord.Create('Состояние: ', mGroupBox, 100);
  mStateInfo.ChangeCoord(mLeft,25);

  //mDestinationInfo := TInfoRecord.Create('Пункт назначения: ', mGroupBox, 100);
  //mDestinationInfo.NextTo(mStateInfo, mDelta);

  //mWaitInfo := TInfoRecord.Create('Простой: ', mGroupBox, 100);
  //mWaitInfo.NextTo(mDestinationInfo, mDelta);

  mWorkInfo := TInfoRecord.Create('Работа: ', mGroupBox, 100);
  mWorkInfo.NextTo(mStateInfo, mDelta);

  mWaitPercent := TInfoRecord.Create('Занятость: ', mGroupBox, 100);
  mWaitPercent.NextTo(mWorkInfo, mDelta);

  mMoneyInfo := TInfoRecord.Create('Заработано: ', mGroupBox, 100);
  mMoneyInfo.NextTo(mWaitPercent, mDelta);

  mGroupBox.Height := mMoneyInfo.InfoLabel.Top + 30;
  mGroupBox.Width := mMoneyInfo.Edit.Left + mMoneyInfo.Edit.Width + 10;
end;
...
procedure TInfoControls.DowntTo(IC: TInfoControls; delta: integer);
begin
  mGroupBox.Top := IC.GroupBox.Top + Ic.GroupBox.Height + delta;
  mGroupBox.Left := IC.GroupBox.Left;
end;
