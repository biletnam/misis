...
begin
  ...
  mTer.mDuration := TMinMaxSelect.Create('Продолжительность приема у '+N2_R+': ', mInputForm);
  ...
  mHir.mDuration := TMinMaxSelect.Create('Продолжительность приема у '+N3_R+': ', mInputForm);
  ...
  mPsi.mDuration := TMinMaxSelect.Create('Продолжительность приема у '+N4_R+': ', mInputForm);
...
begin
  ...
  mLog.Add(mTime, 'Пациент' + IntToStr(Client.Number)+ ' пришел в клинику.');
end;
...
begin
   ...
   begin
      if FindMinQueue(Client).Ctrl.Name = 'Врач 1' then s:='Врачу 1';
      if FindMinQueue(Client).Ctrl.Name = 'Врач 2' then s:='Врачу 2';
      if FindMinQueue(Client).Ctrl.Name = 'Врач 3' then s:='Врачу 3';


      mLog.Add(mTime, 'Пациент' + IntToStr(Client.Number)+ ' пошел к ' + s);
      ClientFromNodeToQueue(From,FindMinQueue(Client));
   end
   else
   begin
      mLog.Add(mTime, 'Пациент' + IntToStr(Client.Number)+ ' ушел из клиники.');
...
begin
   ...
   begin
     ...
     begin
       ...
       mLog.Add(mTime, 'Пациент' + IntToStr(Client.Number)+ ' начал проходить регистрацию.');
     end;
     Client.WaitTimer:= Client.WaitTimer-1;
     if Client.WaitTimer = 0 then
     begin
       // Регистрация пройдена
       mLog.Add(mTime, 'Пациент' + IntToStr(Client.Number)+ ' прошел регистрацию.');
       SetMDs(Client);
   ...
   begin
     ...
     begin
       ...
       mLog.Add(mTime, 'Врач 1 начал прием Пациента' + IntToStr(Client.Number)+'.');
     end;
     Client.WaitTimer:= Client.WaitTimer-1;
     if Client.WaitTimer = 0 then
     begin

       mLog.Add(mTime, 'Врач 1 закончил прием Пациента' + IntToStr(Client.Number)+'.');
   ...
   begin
     ...
     begin
       ...
       mLog.Add(mTime, 'Врач 2 начал прием Пациента' + IntToStr(Client.Number)+'.');
     end;
     Client.WaitTimer:= Client.WaitTimer-1;
     if Client.WaitTimer = 0 then
     begin

       mLog.Add(mTime, 'Врач 2 закончил прием Пациента' + IntToStr(Client.Number)+'.');
   ...
   begin
     ...
     begin
       ...
       mLog.Add(mTime, 'Врач 3 начал прием Пациента' + IntToStr(Client.Number)+'.');
     end;
     Client.WaitTimer:= Client.WaitTimer-1;
     if Client.WaitTimer = 0 then
     begin

       mLog.Add(mTime, 'Врач 3 закончил прием Пациента' + IntToStr(Client.Number)+ '.');
