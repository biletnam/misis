...
begin
  ...
  mClientsDone := TInfoRecord.Create('Обслужено пациентов: ', mGroupBox, 125);
  mClientsDone.ChangeCoord(mLeft,25);

  mClientsNow := TInfoRecord.Create('Сейчас в клинике: ', mGroupBox, 125);
  mClientsNow.NextTo(mClientsDone, mDelta);

  mAvgTotalTime := TInfoRecord.Create('Среднее время'+#10#13+'обслуживания: ', mGroupBox, 125);
  mAvgTotalTime.NextTo(mClientsNow, mDelta);

  AvgWaitTime := TInfoRecord.Create('Среднее время'+#10#13+'ожидания: ', mGroupBox, 125);
...
begin
  ...
  mQueueInfo := TInfoRecord.Create('Очередь: ', mGroupBox, 75);
  mQueueInfo.ChangeCoord(mLeft,25);

  mStateInfo := TInfoRecord.Create('Состояние: ', mGroupBox, 75);
  mStateInfo.NextTo(mQueueInfo, mDelta);

  mWaitInfo := TInfoRecord.Create('Простой: ', mGroupBox, 75);
  mWaitInfo.NextTo(mStateInfo, mDelta);

  mWorkInfo := TInfoRecord.Create('Работа: ', mGroupBox, 75);
  mWorkInfo.NextTo(mWaitInfo, mDelta);

  mWaitPercent := TInfoRecord.Create('Занятость: ', mGroupBox, 75);
