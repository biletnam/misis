...
begin
  ...
  mClientsDone := TInfoRecord.Create('Прошли медкомиссию: ', mGroupBox, 125);
  mClientsDone.ChangeCoord(mLeft,25);

  mClientsNow := TInfoRecord.Create('Сейчас проходят: ', mGroupBox, 125);
  mClientsNow.NextTo(mClientsDone, mDelta);

  mAvgTotalTime := TInfoRecord.Create('Среднее время'+#10#13+'прохождения: ', mGroupBox, 125);
