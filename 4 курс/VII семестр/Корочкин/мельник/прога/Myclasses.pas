...
interface
uses classes, Sysutils, Contnrs, Forms, Windows, Controls, StdCtrls, ComCtrls, Graphics;
...
begin
  ...
  mQueueInfo := TInfoRecord.Create('Очередь: ', mGroupBox, 100);
  mQueueInfo.NextTo(mStateInfo, mDelta);

  mWaitInfo := TInfoRecord.Create('Ср. время'+#10+#13+ 'обслуживания: ', mGroupBox, 100);
  mWaitInfo.NextTo(mQueueInfo, mDelta);
  mWaitInfo.InfoLabel.Font.Size := 10;
  mWaitInfo.Edit.Top := mWaitInfo.Edit.Top - 10;
  mWaitInfo.Edit.Text := '0 сек.';
