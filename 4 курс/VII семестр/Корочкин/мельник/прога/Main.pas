...
begin
  ...
  if TickTimer.Enabled then Button2.Caption:='Пауза'
...
begin
  ...
  (SceneMan.SellerMan.Sellers[0] as TSeller).InfoControls.WaitInfo.Edit.Text := '0 сек';
  (SceneMan.SellerMan.Sellers[1] as TSeller).InfoControls.WaitInfo.Edit.Text := '0 сек';
  (SceneMan.SellerMan.Sellers[2] as TSeller).InfoControls.WaitInfo.Edit.Text := '0 сек';
...
begin
  ...
  begin
    Label2.Caption := 'Обслужено клиентов: '+IntToStr(SceneMan.Table.EndClientsList.Count);
    Label3.Caption := 'Не обслужено клиентов: '+IntToStr(SceneMan.Noserved);
   // Label4.Caption := 'Среднее время обслуживания: '+IntToStr(avg)+' сек.';
    Label6.Caption := 'Max длина очереди: '+IntToStr(max);
