...
begin
  ...
  gp.SetXRange(0,6.7); //содержание углерода в %
  gp.SetYRange(600,1600); //температура
  ...
  begin
    ...
    AddText('Жидкость+Аустенит',Coord(2.2,1275));
    AddText('Жидкость+Цементит',Coord(5.8,1184));
    AddText('Аустенит',Coord(0.7,1125));
    AddText('Феррит',Coord(0.3,765));
    AddText('Аустенит+Цементит',Coord(4.0,875));
    AddText('Феррит, Перлит, Ледебурит, Цементит',Coord(3.3,675));
    AddText('Жидкость',Coord(4.3,1415));
  end;
  gp.Repaint;
end;
...
var s:string;
begin
  Label1.Caption := 'Температура:  '+FloatToStrF(gp.fromY(Y),ffFixed,4,0)+'° C';
  Label2.Caption := 'Концентрация углерода:  '+FloatToStrF(gp.fromX(X),ffFixed,1,1)+' %';
end;
